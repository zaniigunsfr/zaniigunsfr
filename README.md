...
--// window locals
local default_color = Color3.fromRGB(93, 63, 211);
...
if (not (isfolder("zaniiguns"))) then
	makefolder("zaniiguns");
end;
...
local locals = {
	...
	should_zaniiguns_destroy = false,
	old_ticks = {
...
screen_gui.Name = "https://zaniiguns";
...
screen_gui_2.Name = "https://zaniiguns";
...
do
	...
	do
		...
		instances["local_text"] = utility.instance_new("TextLabel", { 
			Name = "https://zaniiguns",
			Parent = screen_gui_2,
			Text = '<font color="rgb(93, 63, 211)">zaniiguns</font><font color="rgb(255, 255, 255)">.club</font>',
    ...
    do
        ...
        instances["target_ui"]["logo"] = utility.instance_new("ImageLabel", {
            Name = "Logo";
            Parent = instances["target_ui"]["info"];
            BackgroundColor3 = Color3.fromRGB(93, 63, 211);
            BackgroundTransparency = 1.0;
            BorderColor3 = Color3.fromRGB(0, 0, 0);
            BorderSizePixel = 0;
    ...
    do
		...
		do
			...
			utility.new_connection(run_service.Heartbeat, function()
				...
				if (enabled) then
					...
					local text = custom_text_enabled and '<font color="' .. text_color .. '">' .. features.get_text(custom_text) .. '</font>' or '<font color="' .. text_color .. '">zaniiguns</font><font color="rgb(255, 255, 255)">.club</font>';
		...
		do
			...
			utility.new_connection(run_service.Heartbeat, function()
				...
				--// zaniiguns classics
				local classics_enabled = flags["anti_aim_zaniiguns_classics_enabled"];
				local classics_keybind = flags["anti_aim_zaniiguns_classics_keybind"];
				local classics_types = flags["anti_aim_zaniiguns_classics"];
				...
				if ((enabled or classics_enabled) and (keybind or classics_keybind) and (not (classics_keybind and classics_enabled and classics_types == "supercoolboi34 Destroyer") or (classics_keybind and classics_enabled and classics_types == "supercoolboi34 Destroyer"))) then
					...
					local types = {
						...
						["supercoolboi34 Destroyer"] = locals.should_zaniiguns_destroy and hrp.CFrame + Vector3.new(0 / 0, 1, math.huge) or hrp.CFrame
					};
			...
			task.spawn(function()
				while task.wait(0.1) do
					locals.should_zaniiguns_destroy = not locals.should_zaniiguns_destroy;
				end;
			end);
...
do
	if (hookmetamethod) then
		...
		local __index; __index = hookmetamethod(game, "__index", LPH_NO_VIRTUALIZE(function(self, property)
			if (not checkcaller() and ((((flags["anti_aim_c_sync_enabled"] or (flags["anti_aim_zaniiguns_classics_enabled"] and (flags["anti_aim_zaniiguns_classics"] == "Destroy Cheaters" or flags["anti_aim_zaniiguns_classics"] == "supercoolboi34 Destroyer"))) and (flags["anti_aim_c_sync_keybind"] or flags["anti_aim_zaniiguns_classics_keybind"]))) and property == "CFrame" and self == local_player.Character.HumanoidRootPart)) then
...
do
	...
	do
		...
		do
			...
			do
				...
				visuals_bullet_tracers:Toggle({Name = "shawn", Flag = "visuals_bullet_tracers_fade_enabled"});
	...
	do
		...
		local zaniiguns_classics = ui.tabs["anti_aim"]:Section({Name = "zaniiguns Classics", Side = "Left", Size = 150});
		...
		--// destroy cheaters
		do
			zaniiguns_classics:Toggle({Name = "Enabled", Flag = "anti_aim_zaniiguns_classics_enabled"});
			zaniiguns_classics:Keybind({Flag = "anti_aim_zaniiguns_classics_keybind", Name = "Keybind", Default = Enum.KeyCode.Y, Mode = "Toggle"});
			zaniiguns_classics:List({Name = "Classics", Flag = "anti_aim_zaniiguns_classics", Options = {"Destroy Cheaters", "supercoolboi34 Destroyer"}, Default = "Destroy Cheaters"});
		end;
	end;
	...
	do --// credits to finobe wtv im way too lazy
		...
		if not isfolder("zaniiguns/configs") then 
		    makefolder("zaniiguns/configs");
		end;

		local function update_config_list()
		    local list = {};
		
		    for idx, file in listfiles("zaniiguns/configs") do
		        local file_name = file:gsub("zaniiguns/configs\\", ""):gsub(".cfg", ""):gsub("zaniiguns/configs/", "");
		...
		cfgs:Button({Name = "Create", Callback = function()
		    ...
		    if config_name == "" or isfile("zaniiguns/configs/" .. config_name .. ".cfg") then
		        return;
		    end;
		    writefile("zaniiguns/configs/" .. config_name .. ".cfg", Library:GetConfig());
		    update_config_list();
		end});
		...
		cfgs:Button({Name = "Save", Callback = function()
		    ...
		    if selected_config then
		        writefile("zaniiguns/configs/" .. selected_config .. ".cfg", Library:GetConfig());
		    end;
		end});
		...
		cfgs:Button({Name = "Load", Callback = function()
		    local selected_config = flags.setting_configuration_list;
		    if selected_config then
		        Library:LoadConfig(readfile("zaniiguns/configs/" .. selected_config .. ".cfg"));
		    end;
		end});
		...
		cfgs:Button({Name = "Delete", Callback = function()
		    local selected_config = flags.setting_configuration_list;
		    if selected_config then
		        delfile("zaniiguns/configs/" .. selected_config .. ".cfg");
		    end;
		    update_config_list();
		end});
